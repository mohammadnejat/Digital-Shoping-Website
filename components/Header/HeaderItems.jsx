...
function HeaderItems ({ title, items }) {
  ...
  return (
    <li
      className='relative px-2 pb-3 cursor-pointer'
      onMouseEnter={() => setHeaderMenuisShow(true)}
      onMouseLeave={() => setHeaderMenuisShow(false)}
    >
      <span>{title}</span>
      <div
        className={`absolute ${
          headerMenuisShow ? '' : 'hidden'
        }  z-[99999] text-white top-6 right-5`}
      >
          ...
          <div className='col-span-6 p-3 bg-gray-100 rounded-xl h-[450px]'>
            <div className='flex items-center justify-between gap-2 '>
              <div className='flex items-center gap-2'>
                ...
                <small className='text-xs'>(قیمت محصول)</small>
              </div>
              <div>
